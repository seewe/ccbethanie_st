...
export default function AdminPage() {
  ...
  const [eventForm, setEventForm] = useState({
    ...
    month: "MAI",
    category: "Culte",
    location: "2401 avenue de l'abbé Giguere, Québec, QC G1C 0H4",
    description: "",
    speaker: "",
  });
  ...
  const handleDeleteSermon = async (id) => {
    if (!window.confirm("Voulez-vous vraiment supprimer cette prédication ?"))
      return;
    try {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-[#121820] text-white p-6 sm:p-8 rounded-3xl shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 border-b-4 border-[#C9862C]">
          <div>
            ...
            <p className="text-xs text-gray-300 mt-1">
              Gérez les prédications, le calendrier d'événements, les intentions
              de prière et les visiteurs inscrits.
            </p>
          </div>
            ...
            {showAddSermon && (
              <div className="bg-white p-6 rounded-2xl border border-amber-200 shadow-md animate-in fade-in space-y-4">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  <h3 className="text-sm font-bold text-[#8F4D12] uppercase tracking-wider">
                ...
                <form onSubmit={handleCreateSermon} className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer">
                      ...
                      />
                      <span>
                        Mettre en avant sur la page d'accueil (Dernier Message)
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEventForm({ ...eventForm, month: e.target.value })
                        }
                        className="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg"
                      />
                    </div>
        ...
        {activeTab === "visits" && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-gray-900">
              Demandes de Première Visite ({visits.length})
            </h2>
            <div className="bg-white rounded-2xl border border-gray-200 overflow-x-auto shadow-sm">
                  ...
                  {visits.length === 0 ? (
                    ...
                    visits.map((v) => (
                      <tr key={v.id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4 text-gray-700">
                          {v.attendeeCount} adulte(s) •{" "}
                          {v.hasChildren
                            ? `Oui (${v.childrenAges || "Âges non précisés"})`
                            : "Sans enfants"}
                        </td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${v.wantsHost ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                          >
        ...
        {activeTab === "prayers" && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-gray-900">
              Sujets de Prière Reçus ({prayers.length})
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {prayers.map((p) => (
                <div
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[#8F4D12]">
                      {p.category}
                    </span>
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${p.isPublic ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-800"}`}
                    >
                  ...
                  <div className="text-[11px] text-gray-500 flex items-center justify-between pt-2 border-t border-gray-100">
                    <span>
                      Par : <strong>{p.authorName}</strong>
                    </span>
        ...
        {activeTab === "contacts" && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-gray-900">
              Messages du formulaire de contact ({contacts.length})
            </h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <h4 className="font-bold text-sm text-gray-900">
                        {c.name} —{" "}
                        <span className="text-xs text-[#8F4D12] font-normal">
                          {c.subject}
                        </span>
                      </h4>
                    ...
                    <div className="text-xs text-gray-500 flex gap-4 pt-1">
                      <span>
                        ...
                        <a
                          href={`mailto:${c.email}`}
                          className="text-[#C9862C] font-semibold"
                        >
                          {c.email}
                        </a>
                      </span>
                  ...
                  {donations.length === 0 ? (
                    ...
                    donations.map((d) => (
                      <tr key={d.id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4">
                          {d.needTaxReceipt ? (
                            <span className="px-2 py-0.5 rounded text-[10px] bg-green-100 text-green-800 font-bold">
                              Requis ({d.address?.city || "Canada"})
                            </span>
                          ) : (
                        ...
                        <td className="p-4 text-gray-500">
                          {new Date(d.createdAt).toLocaleDateString("fr-CA")}
                        </td>
                      </tr>
                    ))
                  )}
